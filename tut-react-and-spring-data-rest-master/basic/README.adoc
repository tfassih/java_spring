...
.src/main/java/com/greglturnquist/payroll/User.java
====
[source,java]
----
include::src/main/java/com/greglturnquist/payroll/User.java[tag=code]
----
...
<3> It uses constructor injection and autowiring to get Spring Data's automatically created `UserRepository`.
...
* `employees` points to an aggregate root for the employee objects defined by the `UserRepository` interface.
...
====
----
$ curl -X POST localhost:8080/api/employees -d "{\"userName\": \"Bilbo\", \"lastName\": \"Baggins\", \"description\": \"burglar\"}" -H "Content-Type:application/json"
{
  "userName" : "Bilbo",
  "lastName" : "Baggins",
  "description" : "burglar",
...
When the state is updated, the `render()` function is invoked by the framework. The employee state data is included in the creation of the `<UserList />` React component as an input parameter.

The following listing shows the definition for an `UserList`:

.src/main/js/app.js - UserList component
====
[source,javascript,indent=0]
----
...
[source,javascript]
----
<User key={employee._links.self.href} data={employee} />
----
====
...
Next, you need to actually define what an `<User />` is, as follows:

.src/main/js/app.js - User component
====
[source,javascript,indent=0]
----
...
Because this component does not manage any state nor deal with user input, there is nothing else to do. This might tempt you to cram it into the `<UserList />` up above. Do not do it! Splitting your app up into small components that each do one job will make it easier to build up functionality in the future.
