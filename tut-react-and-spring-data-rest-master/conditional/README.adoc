...
To support versioning of resources, define a version attribute for your domain objects that need this type of protection. The following listing shows how to do so for the `User` object:
...
The `CreateDialog` created in the previous section was put at the top of the `UserList` because there is only one instance. However, `UpdateDialog` is tied directly to specific employees. So you can see it plugged in below in the `User` React component:

.src/main/js/app.js - User with UpdateDialog
====
[source,javascript,indent=0]
----
...
You put the JSON Schema metadata to good use in building up the `CreateDialog` and the `UpdateDialog` inputs. Do you see other places to use the metadata to makes things more generic? Imagine you wanted to add five more fields to `User.java`. What would it take to update the UI?
