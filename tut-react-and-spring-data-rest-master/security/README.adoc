...
<1> `@RepositoryEventHandler(User.class)` flags this event handler as applying only to `User` objects. The `@HandleBeforeCreate` annotation gives you a chance to alter the incoming `User` record before it gets written to the database.
====
...
With all these modifications in the backend, you can now shift to updating things in the frontend. First of all, you can show an employee's manager inside the `<User />` React component:
